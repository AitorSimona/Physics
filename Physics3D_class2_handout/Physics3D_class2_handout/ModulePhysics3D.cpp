...
#pragma comment( lib, "Box2D/libx86/BulletCollision.lib" )
#pragma comment( lib, "Box2D/libx86/BulletDynamics.lib" )
#pragma comment( lib, "Box2D/libx86/LinearMath.lib" )
#endif
...
bool ModulePhysics3D::Start()
{
...

    {
		...
		/* btTransform transf;
		 transf.setOrigin({ 100.0,0.5,100.0 });*/
...
update_status ModulePhysics3D::Update(float dt)
{
	...
	if(debug == true)
	{
		...
		if(App->input->GetKey(SDL_SCANCODE_1) == KEY_DOWN)
		{
			...
			btTransform transf;
			mat4x4 mat;
			mat.translate(App->camera->Position.x, App->camera->Position.y, App->camera->Position.z);
            transf.setFromOpenGLMatrix(mat.M);
