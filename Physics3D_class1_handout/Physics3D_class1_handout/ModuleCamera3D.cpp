...
update_status ModuleCamera3D::Update()
{
	...
	{
		vec3 aux = { 0,0,-1 };
		App->camera->Move(App->camera->Z*(-1.0f));
		/*aux = { 0,0,0 };
		App->camera->LookAt(aux);*/
	}
	...
	{
		vec3 aux = { -1,0,0 };
		App->camera->Move(App->camera->X*(-1.0f));
		/*aux = { 0,0,0 };
		App->camera->LookAt(aux);*/
	}
	...
	if(App->input->GetMouseButton(SDL_BUTTON_RIGHT) == KEY_REPEAT)
	{
		...
		rotate(App->camera->Y,dx,App->camera->X);

		rotate(App->camera->X,dy,App->camera->Y);

		/*App->camera->LookAt(App->camera->Reference);*/

	   /*App->camera->LookAt(vec3(dx,-dy,0));*/
